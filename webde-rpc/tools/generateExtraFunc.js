...
function builder(ifaces) {
  ...
  buildStub(ifaces.service + 'Stub.js', initObj, ifaces.interfaces, (remote == 'true' ? true : false));
  if (remote == 'true') {
...
function buildStub(filename, initObj, ifaces, remote) {
  ...
  for (var i = 0; i < ifaces.length; ++i) {
    serviceObj[ifaces[i].name] = 'function(' + ifaces[i].in.join(', ') + (ifaces[i].in.length == 0 ? '' : ', ') + 'callback) {' + TODO + '}';
  }
