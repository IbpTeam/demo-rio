...
exports.createItem = function(item, createItemCallBack){
  var db = openDB();
  db.get(SQLSTR.CREATEVIDEO, item.filename, item.postfix, item.size, item.path, item.location, item.createTime, item.lastModifyTime,item.lastAccessTime, item.others, createItemCallBack);
  closeDB(db);
}
...
exports.updateItemValue = function(id,key,value, updateItemValueCallBack){
  var db = openDB();
    console.log("udpate vidoes id : " + id);
