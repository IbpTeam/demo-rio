...
function getnit(initType) {
  if (initType === "theme") {
    ...
    _shutdown_app.path = "./";
    _shutdown_app.iconPath = "img/exit.png";
 ...
 *
 **/
function writeJSONFile(filePath, oTheme, callback) {
  var systemType = os.type();
  if (systemType === "Linux") {
...
function readConf(callback, sFileName) {
  ...
  if (systemType === "Linux") {
    ...
    readJSONFile(sFileDir, function(err, result) {
      callback(err, result);
    })
  } else {
    console.log("Not a linux system! Not supported now!")
...
function writeConf(callback, sFileName, oContent) {
  ...
  if (systemType === "Linux") {
    ...
    writeJSONFile(sFileDir, oContent, function(err, result) {
      callback(err, result);
    })
  } else {
    console.log("Not a linux system! Not supported now!");
...
function readAppMethod(callback, sFileName) {
  ...
  readJSONFile(sFilePath, function(err, result) {
    callback(err, result);
  })
}
