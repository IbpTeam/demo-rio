...
function renameDataByUri(category, sUri, sNewName, callback) {
  ...
  commonDAO.findItems(null, [category], [sCondition], null, function(err, result) {
    ...
    var sOriginPath = result.path;
    var reg_name = new RegExp(sOriginName);
    var sNewPath = sOriginPath.replace(reg, sNewName);
    ...
    fs_extra.move(sOriginPath, sNewPath, function(err) {
      ...
      var sOriginDesPath = sOriginPath.replace(reg_path, '/' + category + 'Des/');
      var sNewDesPath = path.dirname(sOriginDesPath) + sNewName;
      fs_extra.move(sOriginDesPath, sNewDesPath, function(err) {
        ...
        var oUpdataInfo = {
          ...
          category: category,
          filename: sNewName,
          postfix: path.extname(sOriginPath),
