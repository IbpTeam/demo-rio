...
function gen_files_view(files){
    ...
    for(var i=0; i<files.length; i++){
		var file = files[i];
        result.push('<div class="file" data-path="' + file['props'].path + '">');//files[i].path
        if(file['props'].img){
            if(client_type == clienet_local){ 
                result.push('<div class="icon"> <img src="' + file['props'].img + '"></div>');
            }else{
                result.push('<div class="icon"> <img src="' + file['props'].img + '?query=absolute"></div>');
            }
        }else{
            result.push('<div class="icon"> <img src="icons/' + file['props'].suffix + '.png"></div>');
        }
        result.push('<div class="name">' + file['props'].name + '</div>');
        result.push('</div>');
    }
    return result.join('\n');
}
...
//    		  console.log('e.target', e.target.toString());
	...
	this.element.delegate('.file', 'dblclick', function() {
	    ...
	    //var file_path = file_json['props'].path;
...
Folder.prototype.get_callback_data = function(data_json){
	...
	case 'root':
	    for(var i=0; i<data_json.length; i++){
	        ...
	        data_json[i]['props']['suffix'] = 'folder';
	    }
    ...
    case 'root/Contacts':
	    for(var i=0; i<data_json.length; i++){
	        ...
	        data_json[i]['props']['type'] = 'Contacts';
	        data_json[i]['props']['suffix'] = 'Contacts';
	    }
        break;
    case 'root/Pictures':
	    for(var i=0; i<data_json.length; i++){
	        ...
	        data_json[i]['props']['type'] = 'Pictures';
	        data_json[i]['props']['suffix'] = 'Pictures';
	    }
        break;
     case 'root/Videos':
	    for(var i=0; i<data_json.length; i++){
	        ...
	        data_json[i]['props']['name'] = data_json[i]['filename'];
	        //data_json[i]['img'] = '.'+data_json[i]['photoPath'];	        
	        data_json[i]['props']['type'] = 'Videos';
	        data_json[i]['props']['suffix'] = 'Videos';
	    }
        break;
    case 'root/Documents':
	    for(var i=0; i<data_json.length; i++){
	        ...
	        data_json[i]['props']['name'] = data_json[i]['filename'];   
	        data_json[i]['props']['type'] = 'Documents';    
	        data_json[i]['props']['suffix'] = 'Documents';
	    }
        break;
    case 'root/Music':
	    for(var i=0; i<data_json.length; i++){
	        ...
	        data_json[i]['props']['name'] = data_json[i]['filename'];
	        //data_json[i]['img'] = '.'+data_json[i]['photoPath'];	        
	        data_json[i]['props']['type'] = 'Music';  
	        data_json[i]['props']['suffix'] = 'Music';
	    }
        break;
    default:
        break;
