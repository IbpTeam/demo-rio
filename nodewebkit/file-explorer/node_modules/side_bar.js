...
SideBar.prototype.set_filters = function(json){
	...
	if(json.length){
		switch(json[0]['props'].icon)
		{
		    case 'folder':
		        ...
		        break;
		    case 'Contacts':
		        result.push('<input type="button" id="filter_135" value="135开头"/>');
		        break;
		    case 'Pictures':
		        result.push('<input type="button" id="filter_group" value="版本组"/>');
		        break;
		    case 'Music':
		        result.push('<input type="button" id="filter_jay" value="周杰伦"/>');
		        break;
		    case 'Documents':
		        result.push('<input type="button" id="filter_hgj" value="核高基项目"/>');
		        break;
            default:
                break;
		}        
    ...
    self.filters.delegate("input", "click", "value", function(){
        self.emit('do_filter', json);
    });
}

SideBar.prototype.do_filter = function(json){
    window.alert("we come to filter result.");
}
