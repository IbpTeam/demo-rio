...
function gen_files_view(files){
    ...
    for(var i=0; i<files.length; i++){
		var file = files[i];
        result.push('<div class="file" data-path="' + file['props'].path + '">');
        if(file['props'].img){
            if(client_type == clienet_local){ 
            ...
            result.push('<div class="icon"> <img src="icons/' + file['props'].icon + '.png"></div>');
        }
	...
	this.element.delegate('.file', 'mousedown', function(e) {
		...
		case 3:
            ...
            $(popup_menu).on('mouseup', function(e){
                for(var i=0; i<contents.length; i++){
                    if($(e.target).text() == contents[i]){
                        console.log('file '+ $(file).attr('data-path') + ' pop_menu ' + contents[i] + 'is clicked');
                    }                
                }
                $(this).remove();
		        self.element.children('.focus').removeClass('focus');
