...
//                }
                ...
                case '打开':
                    ...
                    case 'folder':
                        ...
                        break;
                    case 'file':
                        getDataSourceByUri(cb_get_data_source_file, file_json.URI);
                        break;
                    }
                    break;
                case '属性':
                    ...
                    case 'file':
                        ...
                        getDataByUri(cb_get_all_data_file, file_json.URI);
                        break;
                    }
                    break;
                }
    ...
    this.files.delegate('.file', 'dblclick', function() {
        var file_json;
        if($(this).attr('data-path').lastIndexOf('.') != -1 || $(this).attr('data-path') == 'root/Contacts' || $(this).attr('data-path') == 'root/Pictures' || $(this).attr('data-path') == 'root/Videos' || $(this).attr('data-path') == 'root/Documents' || $(this).attr('data-path') == 'root/Music'){       
...
Folder.prototype.find_json_by_path = function(filepath){
    ...
    if(all.length){
        for(var i=0; i<all.length; i++){
            if(all[i]['props'].path == filepath || all[i]['path'].indexOf(filepath) != -1){
                file = all[i];
                break;
            }
        }
...
Folder.prototype.use_folder_view_mode = function(){
    ...
    case 'root/Videos':
        ...
        break;
    case 'root/Documents':
        getAllDataByCate(this.folder_view_mode_cb, 'Documents');
        break;
    case 'root/Music':
    ...
    for(var i=0; i<files.length; i++){
        ...
        if(file['props'].img){
            if(frontend_type == frontend_local){ 
                ...
                jimg.attr({
                    'src': file['props'].img + '?query=absolute'
                });
            }
        }else{
            jimg.attr({
